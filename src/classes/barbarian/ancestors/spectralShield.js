async function shieldHelper(token, sourceToken, targetToken, ditem) {
  ...
  let spectralShield = itemUtils.getItemByIdentifier(
    token.actor,
    'ac55eSpectralShield',
  );
  ...
  let selection = await dialogUtils.confirm(
    spectralShield.name,
    genericUtils.format(
      'CHRISPREMADES.Macros.SpiritShield.Damage',
      { item: spectralShield.name, name: targetToken.document.name },
    ),
    { userId: socketUtils.firstOwner(token.actor, true) },
  );
  if (!selection) return;
  let result = await workflowUtils.syntheticItemRoll(
    spectralShield,
    [token],
    {
  ...
  let vengefulAncestors = itemUtils.getItemByIdentifier(
    token.actor,
  ...
  if (vengefulAncestors) {
    ...
    featureData
      ...
      .damage
      .parts[0]
      .bonus = result.damageRolls[0].total;
    await workflowUtils.syntheticItemDataRoll(
      featureData,
      token.actor,
      [sourceToken],
    );
...
async function damageApplication({
  trigger: { sourceToken, targetToken },
  ditem,
}) {
  let nearbyTokens = tokenUtils.findNearby(
    targetToken,
    30,
    'ally',
    { includeIncapacitated: false, includeToken: true },
  );

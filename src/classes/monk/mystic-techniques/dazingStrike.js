...
async function preDazingStrike({
  trigger: { entity: item },
  workflow,
  mysticTechniques,
  practicedStrikes,
...
async function dazingStrike({
  ...
  const usedEffect = await effectUtils.createEffect(item.actor, effectData);
  await workflowUtils.addEntityRemoval(workflow, [usedEffect]);
  return true;
}

async function postDazingStrike({
  trigger: { entity: item },
  mysticTechniques,
  practicedStrikes,
}) {
...
async function dazingStrikeWorkflow({
  trigger: { entity: item },
  workflow,
  ditem,
}) {
  ...
  const res1 = await preDazingStrike({
    trigger: { entity: item },
    workflow,
    ditem,
    mysticTechniques,
  ...
  if (!res1) return;
  const res2 = await dazingStrike({
    trigger: { entity: item },
    workflow,
  });
  if (!res2) return;
  await postDazingStrike({
    trigger: { entity: item },
    mysticTechniques,
    practicedStrikes,
  });
...
export const ac55eDazingStrike = {
  name: 'Dazing Strike',
  version: '1.3.141',
  rules: 'modern',
  midi: {

...
const CHIVALRIC_MARK_DEFAULT_DISTANCE = 10;
const CHIVALRIC_MARK_LEVEL_10_DISTANCE = 30;
...
const preApply = async (
  ...
  if (markEffect && markEffect.origin === feat.actor!.uuid) return false;
  if (!unyieldingKnight) {
...
const duringApply = async (
  ...
  const target = workflow.hitTargets.first()! as Token;
  const maxDistance =
    (feat.actor! as Actor5e).classes['alternate-fighter'].system.levels >= 10
...
const preSource = async (
  ...
  if (actorUtils.hasUsedReaction(token.actor)) {
    ...
    if (!legendaryKnightErrant?.system?.uses?.value) return [];
  }
  const distance = tokenUtils.getDistance(token, sourceToken);
  const meleeWeapons = (
    token.actor.items.filter((i) => i.type === 'weapon') as Item<'weapon'>[]
  )
...
const duringSource = async (
  ...
  await workflowUtils.syntheticItemRoll(selectedWeapon, [sourceToken], {
    userId: socketUtils.firstOwner(token.actor, true),
  });
  return true;
};
...
const movedWorkflow: MacroFunction = async ({
  ...
  if (!markEffect) return;
  if (markEffect.origin !== feat.actor.uuid) return;
  const effectMaxDistance =
    markEffect.flags['alternate-classes-55e']?.macros?.chivalricMark?.distance;

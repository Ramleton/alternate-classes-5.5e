...
import { spendExploitUses } from 'exploits/handling/exploitUtils.js';
import { getAlternateMartialExploitDie, getAltMartialExploitsRemaining } from 'exploits/utils.js';
...
const pre = async (
  ...
  if (token.id === sourceToken.id)
    return { use: false, spendReaction: true };
  if (!token.actor)
    return { use: false, spendReaction: true };
  ...
  if (targetToken.actor.system.attributes.ac.value > workflow.attackTotal)
    return { use: false, spendReaction: true };
  const { utils: { actorUtils, dialogUtils, itemUtils } } = chrisPremades;
  const validEquipment = token.actor.items
    .filter(i => i.system.equipped)
    .filter(i => ['simpleM', 'martialM', 'shield']
      .includes(i.system.type.value),
    );
...
const during = async (
  ...
  if (!exploitDie)
    return false;
  const { utils: { genericUtils, rollUtils } } = chrisPremades;
  ...
  if (!getAltMartialExploitsRemaining(feat))
    return 1;
  const { utils: { dialogUtils, effectUtils } } = chrisPremades;
  const selection = await dialogUtils.confirm(
...
const post = async (token: Token, spendReaction: boolean) => {
  const { utils: { actorUtils, itemUtils } } = chrisPremades;
  if (spendReaction) {

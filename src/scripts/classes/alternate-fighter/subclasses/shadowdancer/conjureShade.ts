...
const use: MidiMacroFunction = async ({ workflow }) => {
  if (!workflow.token)
    return;
  const {
    Summons,
  ...
  if (!teleportFeature || !attackFeature || !dismissFeature)
    return;
  ...
  if (!name?.length)
    name = `${workflow.actor.name}'s Shade`;
  const updates = {
    actor: {
      name,
      img: undefined,
  ...
  if (!tokenImg?.length)
    tokenImg = workflow.token.document.texture.src;
  if (avatarImg)
  ...
  const spawnedTokens = await Summons.spawn(makeTwo
    ? [sourceActor, sourceActor]
    : sourceActor, updates, workflow.item, workflow.token,
  {
    range: 15,
    animation,
  ...
  for (const i of spawnedTokens) {
    if (applyFilter)
      ...
      await TokenMagic.addFilters((i as any).object, filter);
    if (!restorativeShadows)
      continue;
    ...
    await genericUtils.update(
      targetEffect,
      {
        'flags.chris-premades.macros.midi.actor': ['ac55eConjureShadeActive'],
      },
    );
  }
};
const dismiss: MidiMacroFunction = async ({ workflow }) => {
...
async function attack({ workflow }) {
  ...
  const effect = effectUtils.getEffectByIdentifier(
    workflow.actor, 'ac55eConjureShade');
  if (!effect)
    return;
  const sceneShades = effect
    .flags['chris-premades']

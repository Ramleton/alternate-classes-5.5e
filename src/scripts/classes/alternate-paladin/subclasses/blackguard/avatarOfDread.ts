...
const necroticHeal: MidiMacroFunction = async ({
  ...
  if (workflowUtils.getActionType(workflow) === 'heal') return;
...
const auraDamage: MacroFunction = async ({
  ...
  if (!isFrightenedBy) return;
  const auraRadius = getAuraRadius(feat.actor!);
  if (tokenUtils.getDistance(token, target) > auraRadius) return;
  await runActivity(feat, 'damage', [target]);
};

const avatarOfDread: CPRMacro = {

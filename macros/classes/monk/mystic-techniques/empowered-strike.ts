...
async function empoweredStrike({trigger: {entity: item}, workflow, ditem}) {
	...
	const mysticTechniques = itemUtils.getItemByIdentifier(item.actor, 'mysticTechniques');
	if (!mysticTechniques?.system?.uses?.value) return;
	if (!item.system.uses.value) return;
	const selection = await dialogUtils.confirmUseItem(item, {userId: socketUtils.firstOwner(item.actor, true)});
	if (!selection) return;
	const saveActivity = activityUtils.getActivityByIdentifier(item, 'save', {strict: true});
	...
	const saveAdvantage = sizeMap[opponentActor.system.traits.size] > sizeMap[item.actor.system.traits.size];
	...
	const saveWorkflow = await workflowUtils.syntheticActivityRoll(saveActivity, [workflow.hitTargets.first()]);
	...
	await tokenUtils.pushToken(workflow.token, workflow.hitTargets.first(), range);
}
